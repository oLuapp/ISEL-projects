...
void parse_arguments(int argc, char *argv[], client_config_t *config) {
    ...
    while ((opt = getopt_long(argc, argv, "s:p:n:urd:c:a:h", long_options, &option_index)) != -1) {
        switch (opt) {
            case 's':
                strncpy(config->host, optarg, MAX_BUFFER - 1);
                break;
            case 'p': {
                char *endptr;
                errno = 0;
            ...
            case 'd':
                ...
                break;
            case 'c':
                strncpy(config->program, optarg, MAX_BUFFER - 1);
                break;
            case 'a':
                strncpy(config->args, optarg, MAX_BUFFER - 1);
                break;
            case 'h':
                print_usage(argv[0]);
                exit(EXIT_SUCCESS);
...
int main(int argc, char* argv[]) {
    ...
    // Debug output to verify parsed arguments
    // printf("Configuration:\n");
    //  printf("Host: %s\n", config.host);
    // printf("Port: %d\n", config.port);
    // printf("Receive: %s\n", config.receive ? "yes" : "no");
    // printf("Directory: %s\n", config.directory);
    // printf("Program: %s\n", config.program);
    // printf("Args: %s\n", config.args);
    ...
    for (int i = 0; i < config.connections; i++) {
        ...
        while (socket_fd != -1) {
            for (int i = 0; i < config.num_files; i++) {
                ...
                // Open the file
                FILE* file = fopen(config.files[i], "rb");
                if (file == NULL) {
                ...
                char* filename = basename(config.files[i]);
