...
void init_vector_shared(vector_shared_t *shared, int *v, size_t v_sz, int nThreads) {
    ...
    shared->nThreads = nThreads;
    pthread_mutex_init(&shared->mutex, NULL);
    pthread_barrier_init(&shared->barrier, NULL, nThreads);
}
...
void *thread_func(void *argp) {
    ...
    min_max(arg);           //Fase1
    pthread_barrier_wait(&shared->barrier); // Sincronização após Fase1
    ...
    pthread_barrier_wait(&shared->barrier); // Sincronização após Fase2

    if (arg->thread_id == 0) {
        shared->average = shared->global_sum / shared->v_sz;
    }

    pthread_barrier_wait(&shared->barrier); // Sincronização após Soma global
    classify(arg);              //Fase3
    return NULL;
}
