...
void mem_Allocation(threadpool_t *tp, int newSize) {
    ...
    handle_error(thread, "monitor_thread memory allocation failed");
...
work_item_t* work_item_create(wi_function_t func, void *args) {
    ...
    handle_error(wi, "work_item_create memory allocation failed");
    wi->func = func;
    wi-> args = args;
    return wi;
}
...
void sharedBuffer_init(SharedBuffer *sb, int capacity) {
    ...
    handle_error(sb->buffer, "sharedBuffer_init memory allocation failed");
    sb->iGet = 0;
    sb->iPut = 0;
    sb->nElem = 0;
...
void* monitor_thread(void *args) {
    ...
    while (tp->active) {
        ...
        if(currentCapacity > tp->capacityIncreaseThreshold && tp->workers_current < tp->workers_max) {
            ...
            tp->workers_current++;
            printf("Increased number of worker threads to %d\n------------------\n", tp->workers_current);
            ...
            pthread_mutex_unlock(&tp->finished_worker_mutex);
            mem_Allocation(tp, 0);
            printf("Decreased number of worker threads to %d\n------------------\n", tp->workers_current);
        }
    }
    return NULL;
}
...
int threadpool_init(threadpool_t *tp, int queueDim, int nthreads_min, int nthreads_max, int cIncThreshold, int cDecThreshold) {
    ...
    handle_error(tp->sb, "threadpool_init memory allocation failed");
    sharedBuffer_init(tp->sb, queueDim);
    tp->work_threads = malloc(sizeof(pthread_t) * nthreads_min);
    if (tp->work_threads == NULL) return -1;
    handle_error(tp->work_threads, "threadpool_init memory allocation failed");
